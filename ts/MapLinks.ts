...
export class MapLinks {
	...
	private pathsToBeChecked = new Set<string>();
	...
	mapLinks(nodesByExecution: LGraphNode[], rerun = false) {
		...
		if (!rerun) {
			...
			if (changedNodes.size !== 0 || changedLinks.size !== 0) {
				...
				this.pathsToBeChecked = new Set<string>();
	...
	definePathForLink(linkId: string, fast = false) {
		const link = this.canvas.graph?.links[linkId];
		...
		const outputXY = Array.from(outputXYConnection) as Point;
