	...
	initListeners(canvas) {
		...
		if (!this.canvas.canvas) {
			...
			this.canvas.canvas.addEventListener("litegraph:canvas", (e) => {
				const { detail } = e;
				if (
					!this.release_link_on_empty_shows_menu &&
					detail &&
	...
	getCurrentConnection() {
		if (this.isComfyUI) {
			const connectingLink =
				this.canvas.connecting_links && this.canvas.connecting_links.length > 0
