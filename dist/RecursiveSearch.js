...
export class RecursiveSearch {
    ...
    constructor(startPos, endPos, nodes, ctx = null) {
    ...
    search(currentPos) {
        ...
        while (this.candidates.length > 0 && candidatesChecked++ < MAX_ITERATIONS) {
            ...
            if (pathFound) {
                //				return pathFound;
                filterInPlace(this.candidates, (p) => p.price + p.targetEstimate < this.targetFoundPrice);
            }
        }
    ...
    findCandidates(currentCandidate) {
        ...
        if (!currentCandidate.fullyVisited) {
            ...
            this.gridSpaceUsed[currentCandidate.gridX][currentCandidate.gridY] =
                false;
            this.checkCandidate(currentCandidate, currentCandidate.positionFrom, currentCandidate.position, currentCandidate.price);
            return false;
        }
        if (currentPos[0] === this.endPos[0] && currentPos[1] === this.endPos[1]) {
            this.targetFoundPrice = Math.min(this.targetFoundPrice, currentCandidate.price);
        ...
        for (const parentCandidate of candidates) {
            ...
            for (const candidate of candidates) {
                ...
                if (this.gridSpaceUsed[gridX]?.[gridY] &&
    ...
    addCandidate(currentPos, candidate, price) {
        const key = this.getPointKey(candidate);
        const gridX = Math.floor(candidate[0] / GRID_SIZE);
        const gridY = Math.floor(candidate[1] / GRID_SIZE);
        if (this.positionsVisited[key] ||
    ...
    checkCandidate(candidate, currentPos, candidatePos, price) {
        ...
        if (blockingNode) {
            ...
            if (horizontal) {
                const leftIsCloser = Math.abs(currentPos[0] - area[LEFT]) <
                ...
                const downIsCloser = Math.abs(currentPos[1] - area[UP]) >
    ...
    findCandidatesOnLine(from, to, skipOvershoot = false) {
        ...
        const dist = (vertical ? Math.abs(from[1] - to[1]) : Math.abs(from[0] - to[0])) +
