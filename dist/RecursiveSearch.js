...
const filterInPlaceReturnFiltered = (list, filter) => {
    let i, j;
    const filtered = [];
    for (i = 0, j = 0; i < list.length; ++i) {
        if (filter(list[i])) {
...
export class RecursiveSearch {
    ...
    pathsH = [];
    pathsV = [];
    constructor(startPos, endPos, nodes, paths, ctx = null) {
    ...
    candidatesChecked = 0;
    search(currentPos) {
        const fakeFirstPos = [currentPos[0] - 1, currentPos[1]];
    ...
    highstPriceYet = 0;
    getBestCandidate() {
        let leastPrice = Number.MAX_SAFE_INTEGER, leastTargetEstimate = Number.MAX_SAFE_INTEGER, bestIndex = -1, currentIndex = 0;
        for (const candidate of this.candidates) {
            const price = this.positionsVisited[candidate.keyPrice].price +
    ...
    findCandidates(currentCandidate) {
        ...
        if (!currentCandidate.fullyVisited) {
            ...
            // 	false;
            this.checkCandidate(currentCandidate, currentCandidate.positionFrom, currentCandidate.position, this.positionsVisited[currentCandidate.keyPrice].price);
            return false;
        }
        if (currentPos[0] === this.endPos[0] && currentPos[1] === this.endPos[1]) {
            this.targetFoundPrice = Math.min(this.targetFoundPrice, this.positionsVisited[currentCandidate.keyPrice].price);
            if (this.positionsVisited[currentCandidate.keyPrice].price ===
                this.targetFoundPrice) {
        ...
        const oneAxisAligned = this.endPos[0] === currentPos[0] || this.endPos[1] === currentPos[1];
    ...
    checkCandidate(candidate, currentPos, candidatePos, price) {
        ...
        const groupCandidates = filterInPlaceReturnFiltered(this.candidates, (c) => c.group !== candidate.group);
        ...
        if (blockingNodeArea) {
            ...
            if (blockingNodePos[0] === currentPos[0] &&
                blockingNodePos[1] === currentPos[1]) {
                return;
            }
            if (horizontal) {
                const blockingNodeDist = Math.abs(blockingNodePos[0] - currentPos[0]);
                filterInPlace(groupCandidates, (c) => Math.abs(c.position[0] - currentPos[0]) <= blockingNodeDist);
            }
            else {
                const blockingNodeDist = Math.abs(blockingNodePos[1] - currentPos[1]);
                filterInPlace(groupCandidates, (c) => Math.abs(c.position[1] - currentPos[1]) <= blockingNodeDist);
            }
        }
        ...
        const pathPrices = this.getPathPrice(currentPos, blockingNodePos ? blockingNodePos : candidatePos);
        const directionDim = horizontal ? 0 : 1;
        groupCandidates.sort((a, b) => Math.abs(a.position[directionDim] - currentPos[directionDim]) -
            Math.abs(b.position[directionDim] - currentPos[directionDim]));
        const forwards = candidatePos[directionDim] > currentPos[directionDim];
        let pathPrice = 0, pathPosStart = currentPos[directionDim];
    ...
    testPath(from, to) {
        ...
        if (clipped) {
            ...
            if (Math.abs(from[directionDim] - blockingArea[LEFT + directionDim]) >
    ...
    getPathPrice(from, to) {
        ...
        const pathStart = from[directionDim] < to[directionDim]
            ? from[directionDim]
            : to[directionDim];
        const pathEnd = from[directionDim] < to[directionDim]
            ? to[directionDim]
            : from[directionDim];
