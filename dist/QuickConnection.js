...
export class QuickConnection {
    ...
    getCurrentConnection() {
        if (this.isComfyUI) {
            const connectingLink = this.canvas.connecting_links && this.canvas.connecting_links.length > 0
                ? this.canvas.connecting_links[0]
                : null;
            if (connectingLink) {
                return {
    ...
    pointerUp() {
        ...
        if (this.insideConnection && connectionInfo) {
            if (connectionInfo.input) {
                this.insideConnection.node.connect(this.insideConnection.connection_slot_index, connectionInfo.node, connectionInfo.slot);
            }
            else {
                connectionInfo.node.connect(connectionInfo.slot, this.insideConnection.node, this.insideConnection.connection_slot_index);
            }
            return true;
        }
        return false;
        ...
        const addToAccepting = (arr, node) => {
            ...
            for (let c = 0; c < arr.length; ++c) {
                ...
                if (!input.link || this.useInputsWithLinks) {
                    const accept = LiteGraph.isValidConnection(input.type, fromConnection.type);
    ...
    onDrawOverlay(ctx) {
        ...
        if (connectionInfo) {
            ...
            if (!this.acceptingNodes) {
                this.acceptingNodes = this.findAcceptingNodes(connecting, 
            ...
            const linkPos = [pos[0] + buttonShift[0], pos[1] + buttonShift[1]];
            ...
            const isInsideClosePosition = LiteGraph.isInsideRectangle(mouseX, mouseY, linkCloseArea[0], linkCloseArea[1], linkCloseArea[2], linkCloseArea[3]);
            let boxRect = null;
            const textsToDraw = [];
            ...
            if (isInsideClosePosition) {
                ...
                this.acceptingNodes.filter((acceptingNode) => {
                    ...
                    if (this.connectDotOnly) {
                        isInsideRect = LiteGraph.isInsideRectangle(mouseX, mouseY, linkPos[0] - (LiteGraph.NODE_SLOT_HEIGHT / 2) * scale, linkPos[1] - (LiteGraph.NODE_SLOT_HEIGHT / 2) * scale, LiteGraph.NODE_SLOT_HEIGHT * scale, LiteGraph.NODE_SLOT_HEIGHT * scale);
                    }
                    else {
                        isInsideRect = LiteGraph.isInsideRectangle(mouseX, mouseY, isInput ? box[0] : linkPos[0] - LiteGraph.NODE_SLOT_HEIGHT / 2, linkPos[1] - 10, isInput
                            ? linkPos[0] - box[0] + LiteGraph.NODE_SLOT_HEIGHT / 2
                            : rRect[2] + LiteGraph.NODE_SLOT_HEIGHT / 2, rRect[3]);
                    }
                    if (isInsideRect && !this.insideConnection) {
                        ...
                        ctx.beginPath();
                        ctx.arc(linkPos[0], linkPos[1], 6 * scale, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.closePath();
                        ctx.beginPath();
                        ctx.strokeStyle = "#6a6";
                        ...
                        const slotColor = this.canvas.default_connection_color_byType[acceptingNode.connection.type] || this.canvas.default_connection_color.input_on;
                        ctx.fillStyle =
                            slotColor || this.canvas.default_connection_color.input_on;
                        ctx.beginPath();
                ...
                if (boxRect) {
                    ...
                    ctx.roundRect(boxRect[0], boxRect[1], boxRect[2], boxRect[3], 5);
                    ctx.fill();
                    ctx.closePath();
